...
| P1 — squads + operations + brain | ①②③ ✔ · awaiting ④ playtest | brain wired, opt-in (EnableAutoCommander OFF by default = work WITH native AI). Review fixes (5bfefd7): ops complete+free squads, objectives pruned, diff tasking, auto excludes manual-committed. **Autonomy ladder in the BRAIN: Auto / Assisted (propose+confirm, d3e1465) / Manual per-commander+op+squad (8bae8d8). UI drives it COMMANDER-WIDE (MODE button); per-op/per-squad flip from UI = not yet wired (brain honors it, no control surface).** Proximity MatchSquads (81f8284) + HomeBase=force centroid. Air combined-arms steers aircraft when enabled. **Per-OP autonomy UI now wired (AUTO/MANUAL toggle per op-row); native map markers use the game target sprite; auto-production visible in feed; squads listed in HQ.** 118 Core. Deferred: S3 TargetId, S4 executor O(n²), per-SQUAD autonomy from UI (per-op + commander-wide done). |
...
| P5 — autonomy UI + HUD | HQ readout + controls ✔ · awaiting ④ playtest | HqView → RenderHq → panel: live readout + **MODE button (cycle Auto/Assisted/Manual) + Confirm button (authorise top proposal)** wired panel→screen→runtime→service (f030d46). **Commander ON by default** (EnableAutoCommander=true — the war runs if you do nothing). Per-op/squad flip from UI + cockpit HUD = later. ④ = open map, see it run + drive the ladder |
